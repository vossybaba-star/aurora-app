...
function MyProfileSection({
  ...
  // Auto-generate Champions + DMs when both are empty and we have company context
  useEffect(() => {
    if (icpChampions.length > 0 || icpDecisionMakers.length > 0) return;
    if (!caDescription && !caValueProp) return;
    let cancelled = false;
    ...
    fetch("/api/generate-personas", {
      ...
      .then(d => {
        if (cancelled) return;
        if (d.champions?.length)       setIcpChampions(d.champions);
        if (d.decision_makers?.length) setIcpDecisionMakers(d.decision_makers);
      })
      .catch(() => { /* non-fatal */ })
      .finally(() => { if (!cancelled) setGeneratingPersonas(false); });
    return () => { cancelled = true; };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [profile.companyDomain, caDescription, caValueProp]);

  // Auto-generate product descriptions for any product loaded from DB with empty description
  const autoGeneratedProducts = useRef<Set<string>>(new Set());
  useEffect(() => {
    const emptyProducts = Object.entries(caKeyProducts)
      .filter(([name, desc]) => !desc && !autoGeneratedProducts.current.has(name));
    if (!emptyProducts.length) return;
    emptyProducts.forEach(([name]) => {
      autoGeneratedProducts.current.add(name);
      fetch("/api/generate-product-summary", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
        ...
        .then(r => r.json())
        .then(d => { if (d.summary) setCaKeyProducts(prev => ({ ...prev, [name]: d.summary })); })
        .catch(() => { /* non-fatal */ });
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [Object.keys(caKeyProducts).join(",")]);
                          ...
                          <>
                            ...
                            {!caKeyProducts[currentProduct] && !generatingProduct && (
                              <p className="text-[11px] text-muted-foreground italic">
                                Kammie will generate a summary…
                              </p>
                            )}
                          </>
                        )}
        ...
        <SectionCard>
          <div className="space-y-4">
            ...
            <FieldRow label="Champions (mid-level)">
              {generatingPersonas && icpChampions.length === 0 ? (
                <div className="flex items-center gap-2 py-1">
                  <Spinner className="w-3.5 h-3.5" />
                  <p className="text-[11px] text-muted-foreground">Kammie is selecting roles…</p>
                </div>
              ) : (
            ...
            <FieldRow label="Decision makers (C-suite)">
              {generatingPersonas && icpDecisionMakers.length === 0 ? (
                <div className="flex items-center gap-2 py-1">
                  <Spinner className="w-3.5 h-3.5" />
                  <p className="text-[11px] text-muted-foreground">Kammie is selecting roles…</p>
                </div>
            ...
            <FieldRow label="Pain points, goals &amp; objections">
              {(icpChampions.length === 0 && icpDecisionMakers.length === 0) ? (
                <p className="text-[11px] text-muted-foreground italic">
                  {generatingPersonas ? "Kammie is selecting roles…" : "Add champions or decision makers above, then Kammie will generate insight per role."}
                </p>
              ) : (() => {
                const allPersonas    = [...icpChampions, ...icpDecisionMakers];
